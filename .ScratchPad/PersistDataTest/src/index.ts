...
const loadMissingPokemon = async (
    ...
    await Promise.all(
        toLoad.map(async (p) => {
            const entryStale =
                !(checkLastUpdated(p.name) == undefined) &&
                checkLastUpdated(p.name)["last_modified_dts"] > staleByDate;
            ...
            const [pokemonSpeciesData, getRecurve] = await fetchPkmnSpecData(
                pokemonData["species_url"],
                pokemonData["name"]
            );

            if (getRecurve.length > 0 && getVarieties) {
                await loadMissingPokemon(
                    getRecurve,
                    loadStartTime,
                    staleByDate,
                    false
...
const loadData = async () => {
    ...
    const dbStale =
        !(checkMinLastUpdated() == undefined) &&
        checkMinLastUpdated() > staleByDate;
